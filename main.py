...
with open("recursos.txt") as f: # abre el archivo de recursos
    for linea in f.readlines(): # recorre cada línea
        ...
        recursos.append((partes[0], set(partes[1:]))) # guarda el recurso y sus categorías (como set)

tareas = sorted(tareas, key=lambda t: t[1], reverse=True) # ordena las tareas por duración de mayor a menor (heurística para reducir makespan)
